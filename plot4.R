...
# create dates and times in useful format for Plots 2-4
raw_select[,10] <- paste(raw_select[,1],raw_select[,2],sep=" ")
#column 10 is columns one and two concatenated with comma and space between
raw_select$strp <- strptime(raw_select[,10], format="%d/%m/%Y %H:%M:%S")
raw_select$wkdy<- weekdays(as.Date(raw_select$strp))
tbl <- raw_select

#plot 2
#png(filename="plot2.png")
#plot(tbl[,12],tbl[,3],xlab="",ylab="Global Active Power (kilowatts)",pch="")
#lines(tbl[,12],tbl[,3])
#dev.off()

#plot 3
#png(filename="plot3.png")
#plot(tbl[,12],tbl[,7],xlab="",ylab="Energy sub metering",pch="")
#lines(tbl[,12],tbl[,7],col="black")
#lines(tbl[,12],tbl[,8],col="red")
#lines(tbl[,12],tbl[,9],col="blue")
#legend("topright",c("sub_metering_1","sub_metering_2","sub_metering_3"),lty=1,col=c("black","red","blue"))
#dev.off()

#plot 4
png(filename="plot4.png")
par(mfcol=c(2,2),mar(4,4,1,1))
#upper left plot
plot(tbl[,12],tbl[,3],xlab="",ylab="Global Active Power",pch="")
lines(tbl[,12],tbl[,3])
#lower left plot
plot(tbl[,12],tbl[,7],xlab="",ylab="Energy sub metering",pch="")
lines(tbl[,12],tbl[,7],col="black")
...
lines(tbl[,12],tbl[,9],col="blue")
legend("topright",c("sub_metering_1","sub_metering_2","sub_metering_3"),lty=1,col=c("black","red","blue"))
#upper right plot and lower right plot
with(tbl, {
    plot(tbl[,12],tbl[,5],xlab="datetime",ylab="Voltage",pch="",lty=1)
    lines(tbl[,12],tbl[,5])
    plot(tbl[,12],tbl[,4],xlab="datetime",ylab="Global_reactive_power",pch="",lty=1)
    lines(tbl[,12],tbl[,4])
})
dev.off()
